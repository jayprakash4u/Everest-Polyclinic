          ...
          {items.map((item, index) => {
            ...
            return (
              <article
                key={item.title}
                className={cn(
                  "group relative overflow-hidden rounded-xl p-3 sm:rounded-3xl sm:p-6 md:p-7",
              ...
              >
                <span
                  className="pointer-events-none absolute -right-1 -top-2 select-none font-heading text-4xl font-bold leading-none text-slate-900/[0.04] sm:-top-3 sm:text-6xl"
                  aria-hidden
                >
                ...
                <div className="relative">
                  <div className="mb-2.5 inline-flex h-9 w-9 items-center justify-center rounded-xl bg-white text-primary-600 shadow-[0_8px_24px_rgba(26,85,148,0.10)] ring-1 ring-slate-200/80 transition group-hover:scale-[1.02] sm:mb-5 sm:h-14 sm:w-14 sm:rounded-2xl">
                    {item.icon ? (
                      <ServicePageIcon
                        icon={item.icon}
                      ...
                      <FallbackIcon size={18} strokeWidth={1.75} />
                    )}
                  </div>

                  <h3 className="font-heading text-[13px] font-bold leading-snug text-slate-900 sm:text-lg md:text-xl">
                    {item.title}
                  </h3>

                  {item.description ? (
                    <p className="mt-1.5 text-[11px] leading-relaxed text-slate-600 sm:mt-3 sm:text-sm md:text-[15px] md:leading-7">
                      {item.description}
                    </p>
                  ) : null}
                </div>
